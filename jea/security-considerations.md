...
Active Directory 도메인 컨트롤러에서 가상 계정은 기본적으로 *도메인 관리자* 권한을 갖습니다.
도메인 컨트롤러에는 로컬 관리자에 대한 그룹이 없기 때문입니다.
따라서 도메인 컨트롤러의 가상 계정은 도메인 계정이며 다른 컴퓨터에서 사용될 수 있습니다.
도메인 컨트롤러에서 JEA 세션을 구성할 때 네트워크의 다른 컴퓨터를 관리하는 데 사용될 수 있는 명령을 노출하지 않도록 주의해야 합니다.
...
-----------------------------|-------------------------------------|---------------------------------------------------------|--------------------------------------------------
도메인 컨트롤러            | Default                             | 도메인 사용자, '*DOMAIN*\Domain Admins'의 구성원         | 도메인 사용자, '*DOMAIN*\Domain Admins'의 구성원
도메인 컨트롤러            | 도메인 그룹 A 및 B               | 도메인 사용자, '*DOMAIN*\A', '*DOMAIN*\B'의 구성원       | 도메인 사용자, '*DOMAIN*\A', '*DOMAIN*\B'의 구성원
...
**Pass-thru 자격 증명** 실행 계정을 지정하지 않으면 PowerShell에서 연결하는 사용자의 자격 증명을 사용하여 원격 서버에서 명령을 실행합니다.
